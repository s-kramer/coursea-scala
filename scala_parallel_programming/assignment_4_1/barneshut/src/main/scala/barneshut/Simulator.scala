...
class Simulator(val taskSupport: TaskSupport, val timeStats: TimeStatistics) {
  def updateBoundaries(boundaries: Boundaries, body: Body): Boundaries = {
    boundaries.copy(minX = boundaries.minX min body.x,
      maxX = boundaries.maxX max body.x,
      minY = boundaries.minY min body.y,
      maxY = boundaries.maxY max body.y)
  }
  ...
  def computeSectorMatrix(bodies: Seq[Body], boundaries: Boundaries): SectorMatrix = timeStats.timed("matrix") {
    val parBodies = bodies.par
    parBodies.tasksupport = taskSupport
    parBodies.aggregate(new SectorMatrix(boundaries, SECTOR_PRECISION))((acc, b) => acc += b,
  ...
  def eliminateOutliers(bodies: Seq[Body], sectorMatrix: SectorMatrix, quad: Quad): Seq[Body] =
