...
class ListsSuite extends FunSuite {
  ...
  test("sum of list with negative elements") {
    assert(sum((-5 to 5).toList) === 0)
  }

  test("sum of all zeros") {
    assert(sum((1 to 10).map(_ => 0).toList) === 0)
  }
  ...
  test("max of list with identical elements") {
    assert(max(List(1, 1, 1, 1, 1)) === 1)
  }

  test("max of list with numbers with mixed signs") {
    assert(max((-5 to 5).toList) === 5)
  }
