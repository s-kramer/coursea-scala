...
object TimeUsage {
  ...
  def readCsv(rdd: RDD[String]): (List[String], DataFrame) = {
  ...
  // the grader performs the verification using positional arguments, not column names so this doesn't work even though it should
  //    StructType(columnNames.map {
  //      case id@"tucaseid" => StructField(id, DataTypes.StringType, nullable = false)
  //      case other => StructField(other, DataTypes.DoubleType, nullable = false)
  //    })
    StructType(
      StructField(columnNames.head, DataTypes.StringType, nullable = false) ::
        columnNames.tail.map(name => StructField(name, DataTypes.DoubleType, nullable = false))
    )
  ...
  def row(line: List[String]): Row = {
    val rowElements = line.head :: line.tail.map(x => x.toDouble)
    Row(rowElements: _*)
  }
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    ...
    columnNames.foldLeft((List[Column](), List[Column](), List[Column]())) {
      case ((primary, work, leisure), column) if primaryPrefixes
                                                 .exists(
                                                   p => column.startsWith(p)) => (col(column) :: primary, work, leisure)
      case ((primary, work, leisure), column) if workingPrefixes
                                                 .exists(
                                                   p => column.startsWith(p)) => (primary, col(column) :: work, leisure)
      case ((primary, work, leisure), column) if leisurePrefixes
                                                 .exists(
                                                   p => column.startsWith(p)) => (primary, work, col(column) :: leisure)
      case (acc, _) => acc
    }
  }
  ...
  def timeUsageSummary(
    ...
    def computeSex: (Double => String) = x => if (x == 1) "male" else "female"

    val sexProjection: Column = udf(computeSex).apply($"tesex").as("sex")

    val ageCol: Column = $"teage"
    val ageProjection: Column = when(ageCol >= 15 && ageCol <= 22, "young")
                                .when(ageCol >= 23 && ageCol <= 55, "active")
  ...
  def timeUsageGroupedTyped(summed: Dataset[TimeUsageRow]): Dataset[TimeUsageRow] = {
    ...
    .map { case ((working, sex, age), primary, work, other) => TimeUsageRow(working, sex, age, primary, work, other) }
