...
trait GameDef {
  ...
  case class Block(b1: Pos, b2: Pos) {
    ...
    def moveBlock(move: Move): Block = move match {
      case Up => up
      case Down => down
      case Left => left
      case Right => right
    ...
    private val possibleMoves = List(Left, Right, Up, Down)

    /**
      * Returns the list of blocks that can be obtained by moving
      * the current block, together with the corresponding move.
      */
    def neighbors: List[(Block, Move)] = for (move <- possibleMoves) yield (moveBlock(move), move)
